...
import com.teamwizardry.librarianlib.features.container.AutoTransferResult
...
open class SlotTypeGhost(val maxStackSize: Int = 1, val overstack: Boolean = false) : SlotType() {
    ...
    override fun handleClick(slot: SlotBase, container: ContainerBase, dragType: Int, clickType: ClickType?, player: EntityPlayer): ItemStack? {
...
open class SlotTypeRemoveOnly : SlotType() {
    override fun canTake(slot: SlotBase, player: EntityPlayer?, stack: ItemStack) = false
}