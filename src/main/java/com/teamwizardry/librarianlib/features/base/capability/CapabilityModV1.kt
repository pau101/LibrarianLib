 ...
 */
@SaveInPlace
@Deprecated("This probably won't work", ReplaceWith("CapabilityMod"), DeprecationLevel.WARNING)
abstract class CapabilityModV1(val name: ResourceLocation) {
        ...
        fun onDeath(playerCloneEvent: PlayerEvent.Clone) {
            if (playerCloneEvent.isWasDeath)
                for (cap in capabilities.values) {
                    val newCap = playerCloneEvent.entityPlayer.getCapability(cap(), null) as CapabilityModV1?
                    val oldCap = playerCloneEvent.original.getCapability(cap(), null) as CapabilityModV1?
                    if(oldCap != null && newCap != null)
                        newCap.readFromNBT(oldCap.writeToNBT(NBTTagCompound()))
                }
        }

        fun <T : CapabilityModV1> register(capClass: Class<T>, capObj: ICapabilityObjectProvider<T>) {
    ...
    protected open inner class CapabilityModSerializer : ICapabilitySerializable<NBTTagCompound> {
        ...
        override fun <T : Any?> getCapability(capability: Capability<T>, facing: EnumFacing?): T? {
            return if (capability == this@CapabilityModV1.capability) this@CapabilityModV1 as T else null
        }
        ...
        override fun hasCapability(capability: Capability<*>, facing: EnumFacing?): Boolean {
            return capability == this@CapabilityModV1.capability
        }
